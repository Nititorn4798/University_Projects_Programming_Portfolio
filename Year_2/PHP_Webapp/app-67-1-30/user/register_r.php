...
<?php
    clearstatcache();
    $reload_home = '<meta http-equiv="refresh" content="5;url=user_register.php">';
    if (isset($_POST["submit"]) && !empty($_POST["username"]) && !empty($_POST["password"])
        ...
        $user_tel = $_POST['telphone'];
        $user_status = $_POST['status'];
        $user_birthday = $_POST['birthday'];
        $hashpwd =  hash('sha256', $user_password);
        include "../db_connect.php";
        $cmd_query_chk = "SELECT * FROM users WHERE user_username = '$user_username'";
        ...
        if ($record_chk == 0) {
            ...
            (NULL,'$user_username','$hashpwd','$user_fname',
            '$user_address','$user_tel','$user_status','$user_birthday',now(),'Supporter')";
