...
<?php
    ...
    $reload_query = '<meta http-equiv="refresh" content="0;url=main.php">';
    if (isset($_POST["submit"]) && !empty($_POST["username"]) && !empty($_POST["password"])) {
        $user_username = $_POST['username'];
        $user_password = $_POST['password'];
        $hashpwd =  hash('sha256', $user_password);
        include "../db_connect.php";
        $cmd_query_chk = "SELECT * FROM users_data WHERE user_username = '$user_username' AND user_password = '$hashpwd'";
        $cmd_result_chk = mysqli_query($db_connect,$cmd_query_chk);
        mysqli_close($db_connect);
        $record_chk = mysqli_num_rows($cmd_result_chk);
        if ($record_chk == 1) {
            session_start();
            $_SESSION['sess_user'] = $user_username;
