namespace MyAppTools
{
    partial class Form1
    {
        /// <summary>
        ///  Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        ///  Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        ///  Required method for Designer support - do not modify
        ///  the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            cmbTitleName = new ComboBox();
            lblTitleName = new Label();
            SuspendLayout();
            // 
            // cmbTitleName
            // 
            cmbTitleName.FormattingEnabled = true;
            cmbTitleName.Location = new Point(159, 17);
            cmbTitleName.Name = "cmbTitleName";
            cmbTitleName.Size = new Size(139, 23);
            cmbTitleName.TabIndex = 0;
            cmbTitleName.SelectedIndexChanged += this.comboBox1_SelectedIndexChanged;
            // 
            // lblTitleName
            // 
            lblTitleName.AutoSize = true;
            lblTitleName.Location = new Point(103, 20);
            lblTitleName.Name = "lblTitleName";
            lblTitleName.Size = new Size(50, 15);
            lblTitleName.TabIndex = 1;
            lblTitleName.Text = "คำนำหน้า";
            lblTitleName.Click += this.label1_Click;
            // 
            // Form1
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(800, 450);
            Controls.Add(lblTitleName);
            Controls.Add(cmbTitleName);
            Name = "Form1";
            Text = "Form1";
            Load += Form1_Load;
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private ComboBox cmbTitleName;
        private Label lblTitleName;
    }
}
